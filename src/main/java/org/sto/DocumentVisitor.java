...
public class DocumentVisitor extends StoMarkupParserBaseVisitor<Object> {
    ...
    public Metadata getMetadataFromContext(StoMarkupParser.DocumentContext ctx) {
        ...
        for (StoMarkupParser.MetadataContext mCtx : ctx.metadata()) {
            ...
            switch (entry.key()) {
                case "@document_type" -> documentType = entry.value();
                case "@university"    -> university = entry.value();
                case "@faculty"       -> faculty = entry.value();
                case "@department"    -> department = entry.value();
                case "@topic"         -> topic = entry.value();
                case "@student"       -> student = entry.value();
                case "@supervisor"    -> supervisor = entry.value();
                case "@year"          -> year = entry.value();
            }
        }
